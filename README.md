...
X API はリクエスト従量課金。**1投稿あたり約 $0.05**（2026-07 実測：30日で $1.88 / 39リクエスト）。
...
デベロッパーコンソールに**請求サイクルごとの支出上限**がある。
ここに当たると `403 Forbidden: Your monthly spend cap has been reached.` で投稿が止まる。
2026-07-20 に実際これで停止した（上限 $1.00 のまま1日6投稿していたため3〜4日で到達）。

**頻度を変えるときは、先に クレジット残高 と 支出上限 を確認すること。**
コンソール → クレジット → 「支出上限を管理」。
